document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCourses() {
    try {
      const response = await fetch(
        `https://course-manager-backend-updated-2026.onrender.com/api/courses?owner=${userId}`,
        {
          headers: {
            "x-auth": token,
          },
    ...
    document.querySelectorAll(".delete-btn").forEach((button) => {
      button.addEventListener("click", async function () {
        ...
        if (confirm("Are you sure you want to delete this course?")) {
          try {
            const response = await fetch(
              `https://course-manager-backend-updated-2026.onrender.com/api/courses/${courseId}`,
              {
                method: "DELETE",
                headers: {
                  "x-auth": token,
  ...
  editForm.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch(
        `https://course-manager-backend-updated-2026.onrender.com/api/courses/${id}`,
        {
          method: "PUT",
          headers: {
