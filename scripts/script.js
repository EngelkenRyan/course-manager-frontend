document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchCourses(searchTerm = "") {
    const url = searchTerm
      ? `https://course-manager-backend-updated-2026.onrender.com/api/courses?search=${encodeURIComponent(
          searchTerm
        )}`
    ...
    fetch(url, {
      ...
      .then((courses) => {
        ...
        courses.forEach((course) => {
          ...
          courseCard.innerHTML = `
            ...
            <button class="enroll-button" data-id="${course._id}" ${
          ...
          button.addEventListener("click", async function () {
            ...
            try {
              const response = await fetch(
                `https://course-manager-backend-updated-2026.onrender.com/api/courses/${courseId}/enroll`,
                {
                  method: "POST",
                  headers: {
      ...
      .catch((error) => {
        ...
        coursesContainer.innerHTML = `<p>${
          error.message || "Error loading courses. Please try again."
        }</p>`;
      });
  }
